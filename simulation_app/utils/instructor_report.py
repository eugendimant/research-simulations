    ...
    def generate_markdown_report(
        ...
        if open_ended_details:
            ...
            if llm_calls > 0:
                ...
                total_oe_responses = llm_calls + fallback_uses if fallback_uses else llm_calls
                ...
                lines.append("**LLM Provider Details:**")
                lines.append("")
                provider_display = {
                ...
                if not provider_details:
                    lines.append(f"- Active provider: {active_provider}")
                    lines.append(f"- Total API calls: {llm_calls}")

                lines.append("")
                lines.append("**How responses were generated:**")
                lines.append("")
                lines.append("- Responses were generated by a large language model (Llama 3.3 70B) tailored to your study's questions and conditions")
                if pool_size > 0:
                    lines.append(f"- A pool of {pool_size} base responses was pre-generated across sentiment categories")
                lines.append("- Each participant received a unique response via draw-with-replacement with 7-layer deep persona variation")
                lines.append("- Variation layers: word-level edits, sentence restructuring, verbosity/formality/engagement modulation, synonym substitution, and punctuation variation")
                ...
                if fallback_uses > 0:
                    ...
                    if pct > 50:
                        lines.append(f"- **Note:** {fallback_uses} response(s) ({pct:.0f}%) used template fallback due to API limits — consider providing your own API key for higher coverage")
                    else:
                        lines.append(f"- {fallback_uses} response(s) ({pct:.0f}%) used template fallback due to API rate limits")
                else:
                    lines.append("- All responses were successfully generated by the LLM (no template fallback needed)")
            else:
                ...
                lines.append("- To enable AI-generated responses, ensure the app has internet access or provide a free API key (Groq, Cerebras, or OpenRouter)")
            lines.append("")
