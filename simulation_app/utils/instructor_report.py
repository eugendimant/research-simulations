    ...
    def generate_markdown_report(
        ...
        if open_ended_details:
            ...
            if pool_size > 0:
                ...
                # v1.0.6.3: Also check llm_attempts (includes blocked calls)
                # API was called but no usable responses were produced
                lines.append("**Generation approach:** Template-Based (AI providers attempted)")
                lines.append("")
                _call_desc = f"{llm_calls} API call(s)" if llm_calls > 0 else f"{llm_attempts} provider attempt(s)"
                lines.append(f"- {_call_desc} were made but did not produce usable responses")
                if batch_failures > 0:
                    lines.append(f"- {batch_failures} batch failure(s) encountered across all providers")
    ...
    def generate_comprehensive_report(
        ...
        if open_ended_qs:
            ...
            if pool_size > 0:
                ...
                lines.append("**Generation approach:** Template-Based (AI attempted)")
                lines.append("")
    ...
    def generate_html_report(
        ...
        if oe_questions:
            ...
            if pool_size_h > 0:
                ...
                # v1.0.6.3: Also check llm_attempts (includes blocked calls)
                # API was called but no usable responses were produced
                html_parts.append("<p><strong>Generation approach:</strong> Template-Based (AI providers attempted)</p>")
                _call_desc_h = f"{llm_calls_html} API call(s)" if llm_calls_html > 0 else f"{llm_attempts_html} provider attempt(s)"
                html_parts.append(f"<p>{_call_desc_h} were made but did not produce usable responses.</p>")
