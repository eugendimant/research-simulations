    ...
    def generate_markdown_report(
        ...
        if open_ended_details:
            ...
            if pool_size > 0:
                ...
                # v1.0.9.2: Distinguish real HTTP calls from blocked provider probes.
                # llm_attempts includes rate-limited/unavailable early returns that
                # never made an HTTP request.  Only show llm_calls (actual HTTP
                # requests) in user-facing reports; llm_attempts is for admin diagnostics.
                if _is_adaptive_engine:
                    ...
                    lines.append("**Generation approach:** Template-Based (AI providers were rate-limited)")
                lines.append("")
                if llm_calls > 0:
                    lines.append(f"- {llm_calls} API call(s) were made but providers were rate-limited or returned errors")
                else:
                    lines.append("- AI providers were unavailable or rate-limited during this run")
                if batch_failures > 0:
                    lines.append(f"- {batch_failures} batch generation attempt(s) did not succeed")
                lines.append("- All responses were generated using the built-in template engine (225+ research domains)")
        ...
        if open_ended_qs:
            ...
            if pool_size > 0:
                ...
                if _is_adaptive_comp:
                    ...
                    lines.append("**Generation approach:** Template-Based (AI providers were rate-limited)")
                lines.append("")
                lines.append(f"- {llm_calls} API call(s) made but providers were rate-limited or returned errors")
    ...
    def generate_html_report(
        ...
        if oe_questions:
            ...
            if pool_size_h > 0:
                ...
                # v1.0.9.2: Show actual HTTP calls, not inflated provider probes
                if _is_adaptive_h:
                    ...
                    html_parts.append("<p><strong>Generation approach:</strong> Template-Based (AI providers were rate-limited)</p>")
                if llm_calls_html > 0:
                    html_parts.append(f"<p>{llm_calls_html} API call(s) were made but providers were rate-limited or returned errors.</p>")
                else:
                    html_parts.append("<p>AI providers were unavailable or rate-limited during this run.</p>")
                if batch_failures_h > 0:
                    html_parts.append(f"<p>{batch_failures_h} batch generation attempt(s) did not succeed.</p>")
