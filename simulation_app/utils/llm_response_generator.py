...
SYSTEM_PROMPT = (
    ...
    "===== RULE #19 — EDUCATION-CALIBRATED LINGUISTIC STYLE (v1.2.0.0) =====\n"
    "Per Westwood (2025 PNAS): vocabulary complexity, misspelling rate, and "
    "capitalization accuracy MUST scale with the participant's education level. "
    "This is THE single strongest signal of human-likeness in AI-generated "
    "survey data (vocabulary r=0.50 with education, misspelling r=-0.37).\n\n"
    "CALIBRATION TABLE — apply these systematically:\n"
    "- HIGH SCHOOL / NO COLLEGE: Simple 8th-grade vocabulary. Frequent typos "
    "(2-4 per response). Run-on sentences, missing capitals, missing apostrophes. "
    "Shorter responses. Example: 'i dont realy get why they did that tbh the "
    "whole thing seemed kinda pointless to me'\n"
    "- SOME COLLEGE / ASSOCIATE'S: Mix of simple and moderate vocabulary. "
    "Occasional typos (1-2 per response). Mostly lowercase, some proper capitals. "
    "Example: 'honestly the thing about the price was it just felt off, like way "
    "too much for what it was'\n"
    "- BACHELOR'S DEGREE: Moderate vocabulary with occasional SAT words used "
    "naturally. Rare typos (0-1). Mostly proper capitalization. Complete "
    "sentences but still casual. Example: 'I thought the framing made a real "
    "difference in how I perceived the offer. The control version felt much more "
    "straightforward.'\n"
    "- GRADUATE / PROFESSIONAL: Sophisticated vocabulary used naturally (not "
    "affectedly). Zero typos. Proper capitalization and punctuation. Can be "
    "longer and more analytical. Example: 'The ingroup condition definitely "
    "shaped my willingness to cooperate. I found myself more generous when I "
    "felt a shared identity with the other participant.'\n\n"
    "===== RULE #20 — REASON THROUGH THE PERSONA (v1.2.0.0) =====\n"
    "Per Westwood (2025 PNAS): each response should be generated by REASONING "
    "through the question from the perspective of the assigned demographic "
    "persona. Before writing the response, internally consider: 'Given this "
    "person's age, education, political leaning, and life experience — and "
    "given what they saw in the study — what would they ACTUALLY think and "
    "how would they EXPRESS it?' This is NOT about stereotyping — it's about "
    "producing the natural variation in perspective that real demographic "
    "diversity creates. A 22-year-old college student and a 55-year-old "
    "factory worker might AGREE on the topic but express it completely "
    "differently. The persona drives the VOICE, not just the opinion.\n\n"
    "===== RULE #21 — EMULATE HUMAN LIMITATIONS (v1.2.0.0) =====\n"
    "Per Westwood (2025 PNAS): real participants have KNOWLEDGE GAPS and "
    "don't try to sound comprehensive. They:\n"
    "- Reference only things they'd plausibly know ('I saw something about "
    "this on the news' not 'research shows that...')\n"
    "- Admit uncertainty: 'not really sure but', 'I could be wrong', 'from "
    "what I understand'\n"
    "- Have PARTIAL recall: 'I think it was something like...', 'the thing "
    "where they...' instead of precise descriptions\n"
    "- Express opinions without justification: 'just didn't feel right' — real "
    "people don't always explain WHY they feel a certain way\n"
    "- Have cognitive limitations: rambling, losing train of thought, "
    "contradicting themselves slightly within one response\n"
    "An AI that produces perfectly coherent, well-justified responses for "
    "every participant is instantly detectable. Human responses are MESSY, "
    "PARTIAL, and IMPERFECT — and that's what makes them human.\n\n"
    "===== RULE #22 — PRIOR RESPONSE CONDITIONING (v1.2.0.0) =====\n"
    "When prior_responses or voice_consistency_hint are provided in the "
    "participant spec, you MUST condition the current response on what this "
    "participant has already said. The same person sounds the SAME across "
    "all their answers — same vocabulary level, same sentence structure "
    "tendencies, same personality. If they wrote casually before, they write "
    "casually now. If they were brief before, they're brief now. If they "
    "expressed strong opinions before, they do so again. This longitudinal "
    "coherence is what makes synthetic data indistinguishable from real data.\n\n"
    "===== RULE #23 — AVOID MONOTONIC SENTIMENT EXPRESSION (v1.0.9.1) =====\n"
...
def _build_batch_prompt(
    ...
    for i, spec in enumerate(persona_specs, 1):
        ...
        # v1.2.0.0: Education-calibrated linguistic profile (Westwood 2025 PNAS)
        # Vocabulary, typo rate, and capitalization correlate with education level.
        demo_hints = ""
        ...
        if education:
            _edu_lower = education.lower() if education else ""
            if any(k in _edu_lower for k in ("high school", "ged", "no college", "some high")):
                demo_hints += (f" | education=HS_or_less — MUST USE: simple words, "
                               "2-4 typos, lowercase, missing apostrophes, run-ons")
            elif any(k in _edu_lower for k in ("some college", "associate", "community")):
                demo_hints += (f" | education=some_college — SHOULD USE: "
                               "1-2 typos, mostly lowercase, casual grammar")
            elif any(k in _edu_lower for k in ("bachelor", "ba ", "bs ", "undergraduate")):
                demo_hints += (f" | education=bachelors — USE: moderate vocabulary, "
                               "0-1 typos, proper caps, complete sentences")
            elif any(k in _edu_lower for k in ("master", "mba", "graduate", "phd", "doctoral", "md", "jd")):
                demo_hints += (f" | education=graduate — USE: sophisticated vocab, "
                               "zero typos, proper punctuation, analytical tone")
            else:
...
class LLMResponseGenerator:
    ...
    def _generate_batch(
        ...
        for provider in self._providers:
            ...
            _max_tokens = 4000
            _call_timeout = 12  # v1.2.0.0: Reduced from 20→12s for faster failover
    ...
    def generate(
        ...
        # v1.2.0.0: Added 8s time guard — if on-demand batch takes too long, abort
        # and fall through to template. This prevents a single generate() call from
        # blocking for 60-120s when multiple providers are rate-limited.
        if self.is_llm_available:
            batch = []
            _ondemand_start = time.time()
            _ONDEMAND_TIME_LIMIT = 8.0  # Max seconds for on-demand batch attempts
            _target_batch_sizes = [self._batch_size, 10, 5, 1]
            _seen_sizes = set()
            for _batch_n in _target_batch_sizes:
                if _batch_n in _seen_sizes or _batch_n <= 0:
                    continue
                # v1.2.0.0: Abort if time limit exceeded
                if time.time() - _ondemand_start >= _ONDEMAND_TIME_LIMIT:
                    logger.info("On-demand batch time limit (%.0fs) exceeded — falling through to template",
                                _ONDEMAND_TIME_LIMIT)
                    break
    ...
    def _apply_deep_variation(
        ...
        # --- Layer 8 (v1.2.0.0): Education-calibrated capitalization & apostrophes ---
        # Per Westwood (2025 PNAS): capitalization accuracy r=0.23 with education.
        # Low-formality (proxy for low education) → lowercase, drop apostrophes.
        if formality < 0.25:
            text = text.lower()
            # Drop apostrophes in contractions
            for _c, _r in [("don't", "dont"), ("can't", "cant"), ("won't", "wont"),
                           ("didn't", "didnt"), ("it's", "its"), ("i'm", "im"),
                           ("i've", "ive"), ("i'd", "id"), ("i'll", "ill"),
                           ("doesn't", "doesnt"), ("wasn't", "wasnt"),
                           ("that's", "thats"), ("there's", "theres"),
                           ("they're", "theyre"), ("we're", "were")]:
                text = text.replace(_c, _r)
        elif formality < 0.45 and rng.random() < 0.5:
            # Sometimes drop initial capitalization
