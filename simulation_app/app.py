...
if active_page == 3:
    ...
    # Runtime mode control: lets users prioritize speed when provider latency is high.
    if not is_generating and not has_generated:
        _speed_options = ["Balanced quality (default)", "Fast mode (bounded runtime)"]
        _speed_idx = 1 if st.session_state.get("generation_speed_mode", "balanced") == "fast" else 0
        _speed_choice = st.radio(
            "Generation runtime mode",
            _speed_options,
            index=_speed_idx,
            horizontal=True,
            key="generation_speed_mode_radio",
            help="Fast mode reduces LLM prefill budget and permits one-time emergency fallback if providers are too slow.",
        )
        st.session_state["generation_speed_mode"] = "fast" if _speed_choice.startswith("Fast mode") else "balanced"
    ...
    if is_generating and st.session_state.get("_generation_phase", 0) == 1:
        ...
        # Runtime mode knobs (balanced vs fast mode)
        _speed_mode = st.session_state.get("generation_speed_mode", "balanced")
        if _speed_mode == "fast":
            os.environ["LLM_PREFILL_BUDGET_SECS"] = "45"
            # User-requested override: allow emergency fallback path in fast mode.
            st.session_state["allow_template_fallback_once"] = True
        else:
            os.environ["LLM_PREFILL_BUDGET_SECS"] = "120"
        ...
        _last_progress_emit = {"count": -1}
        def _engine_progress_cb(done: int, total: int, stage: str) -> None:
            if total <= 0:
                return
            if done == _last_progress_emit["count"]:
                return
            _last_progress_emit["count"] = done
            _pct = max(25, min(70, int(25 + (done / max(1, total)) * 45)))
            progress_bar.progress(_pct, text=f"Step 2/5 — Generating participant responses... ({done}/{total})")
            status_placeholder.info(f"🔄 Stage: {stage.replace('_', ' ')} — participants initialized: {done}/{total}")
        ...
        engine = EnhancedSimulationEngine(
            ...
            progress_callback=_engine_progress_cb,
        )
