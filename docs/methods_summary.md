...
When available, responses are generated by a large language model (LLM) that receives the full experimental context — study description, condition assignment, and participant persona — and produces natural, question-specific text that mirrors real survey responses. Key features:

1. **Batch generation**: Multiple responses are generated per API call, each tailored to a different participant profile (varying in verbosity, formality, engagement level, and sentiment)
...
5. **Multi-provider failover**: The system supports multiple free LLM providers (Groq, Cerebras, OpenRouter) with automatic failover if one reaches its rate limit
...
> Dimant, E. (2024). Behavioral Experiment Simulation Tool (Version 1.4) [Computer software].
...
*Version 1.4.9 | Proprietary Software | All Rights Reserved*
