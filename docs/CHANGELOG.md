## 2026-02-13 — v1.0.7.3
### Continuous Self-Audit Workflow
- Added `simulation_app/utils/simulation_run_audit.py` for per-run archival + automatic quality auditing.
- Every completed simulation is now saved into its own folder under `data/simulation_runs/` with instructor copy, CSV, metadata, validation results, and logs.
- Added unseen-run scanner with persistent seen-state (`.run_audit_state.json`) and append-only `continuous_improvement_log.txt`.
- Audit rules now flag empty datasets, blank OE cells, low uniqueness/generic OE text, and OE-without-LLM-activity integrity issues.
- Stored archive/audit metadata in simulation metadata and surfaced archive/log paths in the Generate download UI.

### Quality Engineering
- Added tests in `tests/test_simulation_run_audit.py` to enforce required artifact persistence and new-run-only auditing behavior.
- Added implementation plan doc `docs/simulation_run_self_improvement_plan.md` with 5 completed improvement iterations.


- Refinement: run archive now writes `Run_Manifest.json` with file checksums.
- Refinement: added `oe_too_short` audit rule and aggregate `audit_summary.json` output.
- Refinement: scanner ignores hidden folders and tests expanded for these cases.
- Refinement: audit now validates `Run_Manifest.json` presence/parseability and emits manifest integrity warnings.
- Refinement: audit now writes cumulative `issue_trends.json` and returns top recurring issue codes in summary metadata.
## 2026-02-13 — v1.0.7.2
### Bug Fixes
- Fixed Streamlit startup `KeyError` import crash by removing runtime `sys.modules` purge/re-import behavior from `simulation_app/app.py`.
- Version mismatch handling is now warning-only (restart/redeploy guidance) instead of aggressive module cache mutation during boot.

### Reliability
- Added regression test `tests/test_app_import_safety.py` to prevent reintroduction of startup `sys.modules` purge logic.
- Bumped `BUILD_ID` to force safe cache invalidation without in-process module deletion.
